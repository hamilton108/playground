...
@Component
public class CarSearchRedisRepository {
    private final CarSearchRedisDAO carSearchDAO;

    public CarSearchRedisRepository(CarSearchRedisDAO carSearchDAO) {
    ...
    public List<VehicleBrand> getBrands(int year) {
        List<VehicleBrand> result = new ArrayList<>();

        Map<Integer,String> brands = carSearchDAO.getBrands(year);
        for (Map.Entry<Integer, String> entry : brands.entrySet()) {
            VehicleBrand vehicleBrand = new VehicleBrand();
            vehicleBrand.setBrandName(entry.getValue());
            vehicleBrand.setBrandIdentifier(entry.getKey());
    ...
    public List<VehicleListModel> getModels(int brandId) {
